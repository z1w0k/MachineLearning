...
def create_dataset(
    ...
    gaps = torch.rand(num_samples, 1) * 5.0 + 5.0
    ...
    acceleration_start = (history_steps - 3) * dt
    acceleration_time = torch.clamp(times - acceleration_start, min=0.0)
    acceleration_signs = torch.randint(0, 2, (num_samples, 1, 1)) * 2 - 1
    acceleration_strengths = torch.rand(num_samples, 1, 1) * 1.5 + 0.5
    accelerations = acceleration_signs * acceleration_strengths
    acceleration_mask = (scenario_types == 1).reshape(-1, 1, 1)
    x = x + (
        0.5
        * accelerations
        * acceleration_time.reshape(1, 1, -1) ** 2
        * acceleration_mask
    )

    turn_start = (history_steps - 1) * dt
    turn_time = torch.clamp(times - turn_start, min=0.0)
